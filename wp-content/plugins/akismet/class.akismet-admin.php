...
class Akismet_Admin {
	...
	public static function dashboard_stats() {
		...
		echo '<p>'.sprintf( _n(
				...
				$count
			, 'akismet'), 'https://akismet.com', esc_url( add_query_arg( array( 'page' => 'akismet-admin' ), admin_url( isset( $submenu['edit-comments.php'] ) ? 'edit-comments.php' : 'edit.php' ) ) ), number_format_i18n($count) ).'</p>';
	}
	...
	public static function rightnow_stats() {
		if ( $count = get_option('akismet_spam_count') ) {
			$intro = sprintf( _n(
				...
				$count
			, 'akismet'), 'https://akismet.com', number_format_i18n( $count ) );
		} else {
			$intro = sprintf( __('<a href="%s">Akismet</a> blocks spam from getting to your blog. ', 'akismet'), 'https://akismet.com' );
		}
