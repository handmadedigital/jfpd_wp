...
class Akismet {
	...
	public static function http_post( $request, $path, $ip=null ) {
		...
		if ( ! $ssl_disabled && function_exists( 'wp_http_supports') && ( $ssl = wp_http_supports( array( 'ssl' ) ) ) ) {
			$akismet_url = set_url_scheme( $akismet_url, 'http' );
		...
		if ( $ssl && is_wp_error( $response ) ) {
			do_action( 'akismet_http_request_failure', $response );

			// Intermittent connection problems may cause the first http
			// request to fail and subsequent HTTP requests to succeed randomly.
			// Retry the http request once before disabling SSL for a time.
			$response = wp_remote_post( $akismet_url, $http_args );
		...
		if ( $ssl_failed ) {
			...
			do_action( 'akismet_http_disabled' );
		}
